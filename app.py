...
def get_colored_rows(file_bytes, sheet_name, header_idx, target_col_name):
    """Быстро находит строки с цветовой заливкой"""
    colored_rows = set()
    try:
        ...
        if sheet_name in wb.sheetnames:
            ws = wb[sheet_name]
            target_col_idx = None
            header_row_cells = list(ws.iter_rows(min_row=header_idx, max_row=header_idx, values_only=False))
...
def convert_df_to_html_report(total_old, total_new, delta, df_top10):
    ...
    <body>
        ...
        <table>
            <tr>
                <th>№</th><th>Лист</th><th>Статья расходов</th>
                <th class="right">Было (руб.)</th><th class="right">Стало (руб.)</th>
                <th class="right">Изменение (руб.)</th><th class="right">Доля во влиянии на общую разницу</th>
            </tr>
    """
    for idx, row in df_top10.iterrows():
        html += f"""
            <tr>
                <td>{idx}</td><td>{row['Лист']}</td><td>{row['Статья расходов']}</td>
                <td class="right">{row['Было (руб.)']:,.2f}</td><td class="right">{row['Стало (руб.)']:,.2f}</td>
                <td class="right">{row['Изменение (руб.)']:+,.2f}</td><td class="right">{row['Доля во влиянии на общую разницу']}</td>
            </tr>
        """
...
def run_analysis(old_bytes, new_bytes):
    ...
    for sheet in common_sheets:
        ...
        if target_column in df_old.columns and value_column in df_old.columns and target_column in df_new.columns and value_column in df_new.columns:
            colored_old_rows = get_colored_rows(BytesIO(old_bytes), sheet, int(header_row), target_column)
            colored_new_rows = get_colored_rows(BytesIO(new_bytes), sheet, int(header_row), target_column)
    ...
    if all_expenses_changes:
        ...
        st.caption("💡 Подсказка: Откройте скачанный файл в браузере и нажмите Ctrl+P -> выберите 'Сохранить как PDF'.")
    else:
